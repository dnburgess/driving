		...
		$.extend(self, {
			init: {
				...
				run: function(api) {
					...
					self.$element.on('go', function(e, data) {
						if (self.visibleItems === self.itemsLength) {
							data.force = true;
						}
						...
						if (self.options.circular) {
							if (!self.isMoving) {
								...
								if (self.whether_correction > 0) {
									var correction_items = '';
									self.$items = self.$ul.children();
									for (var i = 0; i < self.revise; i++) {
										correction_items += $(self.$items[i]).prop("outerHTML");
									}
									self.$ul.append(correction_items);
								}

								if (self.offsetNext) {
									to = to - self.offset + self.revise;
			...
			build: function() {
				...
				if (this.options.lazyload) {
					for (var k = 0; k < this.itemsLength; k++) {
						$(this.$items[k]).data(namespace + '-item', k);
					}
				}
				if (this.options.circular) {
					var cycleContent = this.$ul.children().clone(true);
				...
				// give init state
				if (this.options.circular) {
					this.revise = this.visibleItems;
					this.whether_correction = this.visibleItems * 3 - this.itemsLength * 2;
					this.offsetPrev = true;
					this.offsetNext = false;
					this.animation.revise();
			...
			fillGutter: function() {
				...
				var gutterSize,
					styles = {};
				if (this.options.circular && (variables.item * this.itemsLength < variables.viewport)) {
					gutterSize = Math.round(((variables.viewport / this.itemsLength - variables.item) / 2));
					this.visibleItems = this.itemsLength;
				} else {
					gutterSize = Math.round(((variables.viewport / this.visibleItems - variables.item) / 2));
				}
				...
				this.offset = this.current - this.visibleItems;
				this.revise = this.navItems;
			},
			pager: {
				setup: function() {
					// init
					...
					self.$pager.on('click', 'li', function() {
						...
						if ($(this).index() === self.pager.$items.length - 1 && !self.options.circular) {
							...
							if ((navItemsMirror > 0) && (navItemsMirror <= 3)) {
								...
								self.revise = navItemsMirror;
								self.offset -= self.revise;
							} else {
								...
								if (navItemsMirror < 0) {
									...
									self.revise = self.itemsLength - self.view;
			...
			responsive: {
				...
				resize: function() {
					...
					if (!self.options.circular) {
						if (self.view + self.visibleItems > self.itemsLength) {
							self.view = self.itemsLength - self.visibleItems;
						} else if (self.view + self.visibleItems < self.current) {
						...
						var revise = prevVisiableItems - self.visibleItems;
						if (revise > 0) {
							self.offsetNext = true;
							self.offsetPrev = false;
							self.revise = revise;
							self.animation.revise();
						} else if (revise < 0) {
							self.offsetNext = false;
							self.offsetPrev = true;
							self.revise = -revise;
			...
			animation: {
				...
				revise: function() {
					var i;
					if (self.offsetNext) {
						for (i = 0; i < self.revise; i++) {
							self.$items = self.$ul.children();
							if (self.whether_correction > 0) {
								self.$items.first().remove();
							} else {
			...
			touch: {
				...
				revise: function(value, valueMirror) {
			...
			lazyload: {
				setup: function() {
					self.$items.each(function() {
						...
						if ($img.length) {
							$item.addClass(namespace + '_loading');
							self.lazyload.preload($item, $img, $img.length);
						}
					});
				},
				preload: function($item, $imgs, length) {
					...
					$imgs.each(function() {
						...
						var loaded = function() {
							count++;
							if (count === length) {
								$item.removeClass(namespace + '_loading');
							}
							$img.data('lazyload', 'loaded');

							$img.removeAttr("data-src");

							if (typeof self.options.afterLazyLoad === "function") {
								self.options.afterLazyLoad.apply(this, [self.$element]);
							}
						};

						if (typeof $img.data('src') !== 'string') {
							loaded();
						} else if (self.lazyload.isCompleted(this)) {
							loaded();
						} else {
